...
import com.cogmedsys.hsp.api.ucs.config.UcsClientConfiguration;
...
public class MessageService {
     ...
     */
    private final UCSAlertingBroadcaster alertBroadcaster;
    
    /**
     * Message listeners registered against the UCS session
     */
    private final UCSMessageBroadcaster messageBroadcaster;
    
    /**
     * Reference to configuration class for UCS session configuration.
     */
    private UcsClientConfiguration config;
    
    public MessageService() {
        alertBroadcaster = new UCSAlertingBroadcaster();
        messageBroadcaster = new UCSMessageBroadcaster();
    }
    ...
    public void init() {
        try {
            ...
            session = new UCSNiFiSession.UCSNiFiSessionBuilder().withAlertingCommandURL(alertingCommandURL)
                    .withClientCommandURL(clientCommandURL).withUCSClientHost(config.getClientHost())
                    .withUCSClientPort(Integer.parseInt(config.getClientPort())).withUCSAlertingHost(config.getClientHost())
                    .withUCSAlertingPort(Integer.parseInt(config.getAlertingPort()))
                    .withManagementHost(config.getClientHost())
                    .withManagementPort(Integer.parseInt(config.getManagementPort()))
                    .withConversationHost(config.getClientHost())
                    .withConversationPort(Integer.parseInt(config.getConversationPort()))
                    .withUCSClientListener(messageBroadcaster).withUCSAlertingListener(alertBroadcaster).build();//TODO Create singleton session with broadcast
